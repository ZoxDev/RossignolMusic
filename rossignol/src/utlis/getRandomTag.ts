const getRandomTag = async () => {
    ...
    const randomPage : number = Math.floor(Math.random() * getAllThePages.tags['@attr'].totalPages) + 1;
    ...
    const getRandomTagIndex : number = Math.floor(Math.random() * getTagsOnPage.tags.tag.length);
    const tagName : string = getTagsOnPage.tags.tag[getRandomTagIndex].name;
